...
.PHONY: build-cli
build-cli: fmt vet ## Build the rp CLI binary.
	go build -o bin/rp ./cmd/cli

.PHONY: helm-package
helm-package: ## Package the Helm chart into bin/.
	helm package charts/reactive-policy --destination bin/
...
docker-buildx: ## Build and push docker image for the manager for cross-platform support
	...
	- $(CONTAINER_TOOL) buildx create --name reactive-policy-builder
	$(CONTAINER_TOOL) buildx use reactive-policy-builder
	- $(CONTAINER_TOOL) buildx build --push --platform=$(PLATFORMS) --tag ${IMG} -f Dockerfile.cross .
	- $(CONTAINER_TOOL) buildx rm reactive-policy-builder
	rm Dockerfile.cross
