...
import (
	...
	"github.com/Vedooo/reactive-policy/internal/controller"
	webhookreactivepolicyiov1alpha1 "github.com/Vedooo/reactive-policy/internal/webhook/v1alpha1"
...
// RBAC for the built-in action plugins, mirroring their RequiredPermissions and
// aggregated by internal/action.AggregatePermissions (docs/ARCHITECTURE.md §3.3).
// notify.slack:
// +kubebuilder:rbac:groups="",resources=secrets,verbs=get
// k8s.annotate (target kind is policy-defined, hence broad):
// +kubebuilder:rbac:groups="*",resources="*",verbs=get;list;patch;update
// argocd.suspend:
...
func init() {
	...
	utilruntime.Must(reactivepolicyiov1alpha1.AddToScheme(scheme))
...
func main() {
	...
	mgr, err := ctrl.NewManager(ctrl.GetConfigOrDie(), ctrl.Options{
		...
		LeaderElectionID:       "b6a17773.reactive-policy.io",
	...
	if os.Getenv("ENABLE_WEBHOOKS") != "false" {
		if err = webhookreactivepolicyiov1alpha1.SetupReactivePolicyWebhookWithManager(mgr, action.Default()); err != nil {
	...
	setupLog.Info("registered action plugins",
		"plugins", len(action.All()), "rbacRules", len(action.AggregatePermissions(action.All())))
